  ...
  //  if ((password?.isEmpty)! || (fullName?.isEmpty)! || (organization?.isEmpty)!) {
    ...
    if ((email.isEmpty) || (password?.isEmpty)! || (fullName?.isEmpty)! || (organization?.isEmpty)!) {
        ...
        displayMyAlertMessage(userMessage: "All fields are required");
         ...
         displayMyAlertMessage(userMessage: "Password must contain one alphabet and one special character, between 6-16 characters");
    } else if (!isValidEmail(email: email)){
            displayMyAlertMessage(userMessage: "Email is not Valid");
    } else {
           displayMySuccessMessage(successMessage: "Registration is succssssful!");
            self.view.endEditing(true)
